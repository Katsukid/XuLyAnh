...
def get_id_numbers_text(img):
    text = pytesseract.image_to_string(img, lang='vie', config='--psm 7 -c tessedit_char_whitelist=1234567890') # can xoa dau :, cac ky tu la
    # print(text)
    text = text.replace(" ","").split(":")[1][0:7]
    return text
def get_time(img):
    text = pytesseract.image_to_string(img, lang='vie', config='--psm 7 -c tessedit_char_whitelist=1234567890') # can xoa dau :, cac ky tu la
    return text
...
def get_name(img):
    text = pytesseract.image_to_string(img, lang='vie', config='--psm 7 -c tessedit_char_whitelist=ABCDEFGHIJKLMNOPQRSTUWVXYZÁÀẢÃẠẤẦẪẨẬĂẰẮẴẲẶƠỜỚỞỠỢÔỒỐỔỖỘƯỪỨỬỰỮÍÌĨỊỈÙÚỦỤŨÈÉẺẸẼYÝỲỸỴỶÊỀẾỂỄỆĐ') # can xoa dau :, cac ky tu la
    return text